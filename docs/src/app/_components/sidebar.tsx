...
export const Sidebar: FC = () => {
  ...
  // Filter docsDirectories based on current version in pathname
  const pathname = usePathname();
  const versionMatch = pathname.match(/^\/docs\/([^/]+)/);
  const currentVersion = versionMatch?.[1];
  const filteredDirectories = docsDirectories.filter((dir) => {
    const route = dir.route || "";
    // Ensure it starts with /docs/<version> or /docs/latest
    return (
      route.startsWith(`/docs/${currentVersion}`) ||
      (currentVersion === "latest" && route.startsWith(`/docs/beta`)) // if beta is the latest
    );
  });
